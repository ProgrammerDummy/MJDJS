...
impl WorkerService for MyWorkerService {
    ...
    async fn request_work(&self, request: Request<proto::AssignedWorkerId>) -> Result<Response<proto::RequestWorkResponse>, Status> {
        ...
        let mut target_uuid: uuid::Uuid;

        if let Some(worker) = self.scheduler_state.workers.get(&job_requesting_worker_id) {
            worker_capabilities = worker.capabilities.clone();

            if worker.max_concurrent_jobs <= worker.assigned_jobs.len() as u32 {
                return Err(Status::cancelled(format!("no space in worker id: {} job pool for more jobs", job_requesting_worker_id)))
            }
        ...
        //add check here that worker has vacancy in job pool\
        //is this unneccessary due to the pull based mechanism?
        ...
        if let Ok(mut job_queue_guard) = self.scheduler_state.job_queue.lock() {
            ...
            if let Some(job) = extractor.next() { //executes the extractor, if there is a job whose requirements match the worker's capabilities
                ...
                if let Some(mut worker_info) = self.scheduler_state.workers.get_mut(&job_requesting_worker_id) {
                    worker_info.assigned_jobs.insert(job.id);
                ...
                maybe after a few retries, then it should be sent back
